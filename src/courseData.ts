...
INSERT INTO shop_orders (order_no, customer_name, email, city, status, total_amount, paid_at, shipped_at, created_at, note) VALUES
  ('NW-1001', 'Alice Chen', 'alice@example.com', 'Taipei', 'paid', 1280.00, '2026-08-01 10:20:00', NULL, '2026-08-01 10:05:00', '等待倉庫揀貨'),
  ('NW-1002', 'Bob Lin', 'bob@example.com', 'Taichung', 'shipped', 2500.00, '2026-08-01 11:40:00', '2026-08-02 09:10:00', '2026-08-01 11:30:00', '已寄出'),
  ('NW-1003', 'Cara Wu', 'cara@example.com', 'Kaohsiung', 'pending', 890.00, NULL, NULL, '2026-08-02 09:15:00', '等待付款'),
  ('NW-1004', 'Daniel Ho', 'daniel@example.com', 'Taipei', 'paid', 4200.00, '2026-08-02 12:00:00', NULL, '2026-08-02 11:46:00', '高單價商品'),
  ('NW-1005', 'Emma Tsai', 'emma@example.com', 'Tainan', 'cancelled', 650.00, NULL, NULL, '2026-08-02 14:05:00', '客戶取消'),
  ('NW-1006', 'Finn Kao', 'finn@example.com', 'Taipei', 'packed', 3100.00, '2026-08-03 09:30:00', NULL, '2026-08-03 09:12:00', '待安排物流'),
  ('NW-1007', 'Grace Liu', 'grace@example.com', 'Taichung', 'paid', 760.00, '2026-08-03 16:45:00', NULL, '2026-08-03 16:30:00', '一般訂單'),
  ('NW-1008', 'Hank Wang', 'hank@example.com', 'Hsinchu', 'refunded', 1200.00, '2026-08-04 10:00:00', NULL, '2026-08-04 09:42:00', '已退款'),
  ('NW-1009', 'Ivy Yu', 'ivy.order@example.com', 'Taipei', 'shipped', 5600.00, '2026-08-04 13:25:00', '2026-08-05 08:50:00', '2026-08-04 13:10:00', '大額已出貨'),
  ('NW-1010', 'Jack Pai', 'jack@example.com', 'Tainan', 'paid', 9999.00, '2026-08-05 20:20:00', NULL, '2026-08-05 20:05:00', '大額待審核'),
  ('NW-1011', 'Kelly Huang', 'kelly@example.com', 'Kaohsiung', 'pending', 430.00, NULL, NULL, '2026-08-06 08:30:00', '等待付款'),
  ('NW-1012', 'Leo Sun', 'leo.order@example.com', 'Taipei', 'packed', 1800.00, '2026-08-06 09:20:00', NULL, '2026-08-06 09:05:00', '待出貨');
...
FROM credit_packages;`,
  ...
  },
  {
    id: "ecommerce-case",
    section: "情境任務",
    title: "05 Northwind Lite：電商訂單巡查",
    label: "W5-05",
    lead:
      "這一關開始不是憑空寫資料，而是先由系統推薦一份既有電商資料集。學生先看懂資料長相，再做查詢和小幅修改。",
    mission:
      "LiveFit 想參考電商後台的訂單流程。海姐先把一份 Northwind Lite 風格的 shop_orders 訂單資料集放進沙箱，請你先巡查資料，再完成付款、打包、出貨與客服測試單任務。",
    coachLine:
      "情境題先不要急著猜答案。先看資料表有哪些欄位、狀態有哪些值，再依照任務改一小段 SQL。",
    teaching:
    ...
    exercises: [
      {
        id: "w5-05-01",
        title: "先看電商訂單資料集",
        description: "先不要解任務。請查看 shop_orders 前 8 筆資料，觀察欄位和訂單狀態。",
...
LIMIT 8;`
        ...
        successMessage: "很好，你先看過資料集了。接下來的題目就不是憑空猜欄位，而是從既有資料往下處理。",
        referenceSql: `SELECT *
FROM shop_orders
LIMIT 8;`,
        compare: { order: true, columns: true },
      ...
      },
      {
        id: "w5-05-02",
        title: "找已付款待處理訂單",
        description: "倉庫要先處理已付款訂單。回傳 order_no、customer_name、status，篩選 status 是 paid 的訂單。",
        tags: ["WHERE", "訂單狀態"],
        type: "query",
...
WHERE status = 'paid';`
        ...
        successMessage: "漂亮，你找到已付款訂單了。這就是後台清單常見的第一層篩選。",
...
WHERE status = 'paid';`,
      ...
      },
      {
        id: "w5-05-03",
        title: "找已打包待寄出的訂單",
        description: "物流同事要看已打包但還沒寄出的訂單。回傳 order_no、customer_name、city、status，篩選 status 是 packed。",
        tags: ["WHERE", "物流"],
        type: "query",
...
WHERE status = 'packed';`
        ...
        successMessage: "很好，你把 packed 訂單挑出來了，倉庫就知道哪些單可以進入出貨流程。",
...
WHERE status = 'packed';`,
      ...
      },
      {
        id: "w5-05-04",
        title: "大額訂單優先審核",
        description: "主管要先看金額 5000 以上的訂單。回傳 order_no、customer_name、total_amount，依 total_amount 由大到小排序。",
        tags: ["WHERE", "ORDER BY"],
        type: "query",
...
ORDER BY total_amount DESC;`
        ...
        successMessage: "穩，你把大額訂單排出優先順序了。這種查詢很常出現在營運後台。",
...
ORDER BY total_amount DESC;`,
      ...
      },
      {
        id: "w5-05-05",
        title: "最新三筆付款訂單",
        description: "客服要看最近付款的三筆 paid 訂單。回傳 order_no、customer_name、paid_at，依 paid_at 由新到舊排序並只取 3 筆。",
        tags: ["ORDER BY", "LIMIT"],
        type: "query",
...
LIMIT 3;`
        ...
        successMessage: "很好，你用 ORDER BY 和 LIMIT 做出客服常用的最新訂單清單。",
...
LIMIT 3;`,
      ...
      },
      {
        id: "w5-05-06",
        title: "把付款單改成已打包",
        description: "NW-1001 已經完成揀貨。請把這筆訂單的 status 從 paid 改成 packed。",
        tags: ["UPDATE", "單筆修改"],
        type: "mutation",
...
WHERE order_no = 'NW-1001';`
        ...
        successMessage: "漂亮，你只更新了 NW-1001 的狀態，沒有碰到其他訂單。",
        requires: ["UPDATE", "WHERE"],
        checks: [
          {
...
WHERE order_no = 'NW-1001';`,
      ...
      },
      {
        id: "w5-05-07",
        title: "寄出已打包訂單",
        description: "NW-1006 已交給物流。請把 status 改成 shipped，並把 shipped_at 設為 2026-08-06 15:30:00。",
        tags: ["UPDATE", "多欄位修改"],
        type: "mutation",
        starterSql: `UPDATE shop_orders
SET status = 'shipped'
...
WHERE order_no = 'NW-1006';`
        ...
        successMessage: "很好，你完成了多欄位 UPDATE，這很像後台出貨按鈕背後會做的資料修改。",
        requires: ["UPDATE", "WHERE"],
        checks: [
          {
...
WHERE order_no = 'NW-1006';`,
      ...
      },
      {
        id: "w5-05-08",
        title: "新增客服測試訂單",
        description: "客服要新增一筆測試單。新增 order_no NW-1013、customer_name Momo Test、email momo.test@example.com、city Taipei、status pending、total_amount 0、created_at 2026-08-07 10:00:00、note 客服測試單。",
        tags: ["INSERT", "客服測試"],
        type: "mutation",
...
VALUES ('NW-1013', 'Momo Test', 'momo.test@example.com', 'Taipei', 'pending', 0, '2026-08-07 10:00:00', '客服測試單');`
        ...
        successMessage: "很好，你新增了一筆客服測試單。學生到這裡已經把看資料、查資料、改資料、補資料串起來了。",
        requires: ["INSERT"],
        checks: [
          {
...
WHERE order_no = 'NW-1013';`,
      ...
      },
      {
        id: "w5-05-09",
        title: "找需要客服追蹤的訂單",
        description: "客服想先看還沒完成或有問題的訂單。回傳 order_no、customer_name、status，篩選 status 是 pending、cancelled、refunded，依 id 排序。",
        tags: ["WHERE", "IN"],
        type: "query",
        starterSql: `SELECT order_no, customer_name, status
FROM shop_orders
...
ORDER BY id;`
        ...
        successMessage: "很好，你把需要客服追蹤的訂單整理出來了，這是營運後台常見的工作清單。",
        referenceSql: `SELECT order_no, customer_name, status
FROM shop_orders
WHERE status IN ('pending', 'cancelled', 'refunded')
ORDER BY id;`,
      ...
      },
      {
        id: "w5-05-10",
        title: "取消逾時未付款訂單",
        description: "NW-1011 已逾時未付款。請把 status 改成 cancelled，note 改成 逾時未付款。",
        tags: ["UPDATE", "單筆修改"],
        type: "mutation",
        starterSql: `UPDATE shop_orders
...
WHERE order_no = 'NW-1011';`
        ...
        successMessage: "漂亮，你完成了單筆訂單的取消與備註更新，沒有影響其他訂單。",
        requires: ["UPDATE", "WHERE"],
        requiredPatterns: [
          {
